...
test.describe('Employee Portal - Authentication & Access Gate (TDD)', () => {
  ...
  test('TC-AUTH-001: should display password gate with prompt and required password input', async ({
    employeePage,
  }) => {
  ...
  test('TC-AUTH-002: should prevent access when invalid password is submitted', async ({
    employeePage,
  }) => {
  ...
  test('TC-AUTH-003: should submit configured ZELLA_PASSWORD and handle authentication attempt', async ({
    employeePage,
    appConfig,
    page,
  }) => {
  ...
  test('TC-AUTH-004: should allow user to navigate back to Home from password gate', async ({
    employeePage,
    page,
  }) => {
